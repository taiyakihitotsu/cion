...
export type Str<S, R extends string = ""> =
...
// vector?
export type LispIsVector<S> = 
  S extends [['vec', ...infer _]]
    ? ['prim', true]
  : ['prim', false]

// map?
export type LispIsMap<S> = 
  S extends [['map', ...infer _]]
    ? ['prim', true]
  : ['prim', false]

// fn?
export type LispIsFn<S> = 
  S extends [['fn', ...infer _]]
    ? ['prim', true]
  : ['prim', false]

// keyword?
export type LispIsKeyword<S> = 
  S extends [['key', infer _]]
    ? ['prim', true]
  : ['prim', false]

// ifn?
export type LispIsIfn<S> = 
  LispIsKeyword<S> extends ['prim', false]
    ? LispIsFn<S>
  : ['prim', true]

// pos-int?
export type LispIsPosInt<S> = 
  S extends [['prim', infer N extends string]]
    ? N extends `1${infer _}`
      ? ['prim', false]
    : LispIsNumber<S>
...
// odd?
export type LispIsOdd<S> = 
  S extends [['prim', infer N extends string]]
    ? ['prim', IsOdd<N>]
  : ['prim', false]

// even?
export type LispIsEven<S> = 
  S extends [['prim', infer N extends string]]
    ? ['prim', IsEven<N>]
  : ['prim', false]

// zero?
export type LispIsZero<S> = 
  S extends [['prim', infer N extends string]]
    ? N extends '0'
      ? ['prim', true]
    : ['prim', false]
...
// symbol?
export type LispIsSymbol<S> = 
  S extends [['sym', infer _]]
    ? ['prim', true]
  : ['prim', false]

// empty?
export type LispIsEmpty<S> = 
...
export type LispConcat<S, R extends unknown[][] = []> =
...
export type IsKeyMapSexpr<S, env = [[]]> = 
...
export type First<V> = V extends Vector & [`vec`, infer H, ...infer _]
  ? H
  : V extends ['vec']
    ? TNil
...
export type Rest<V> = V extends Vector & [`vec`, infer _, ...infer T]
  ? T[0] extends Atom
    ? [`vec`, ...T]
    : [`vec`]
  : ErrorCase<RestError0, CommonArgVecErrMsg, V>
export type LispRest<S> = 
  S extends [infer V extends Vector] ? Rest<V> : ErrorCase<RestError1, CommonArgVecErrMsgFn<'rest'>, S>
...
export type Conj<V, E> = E extends Atom
  ? V extends Vector
    ? [...V, E]
...
export type Concat<V, W> = V extends Vector
...
export type Take<N extends string, V extends unknown[], R extends unknown[] = []> =
  V extends []
    ? R
...
export type Drop<N extends string, V extends unknown[], R extends unknown[] = []> =
  V extends []
    ? R
...
type _FMap<F, V> = V extends Vector
...
export type Interleave<V, W> = V extends [infer HeadV, ...infer TailV]
...
export type Reduce<F,Init,V> =
  V extends ['vec', ...infer v]
    ? _Reduce<F,Init,v>
    : ErrorCase<ReduceError1, '', [F,Init,V]>
export type LispReduce<S> = 
...
export type Reverse<V, R extends Array<unknown> = []> = 
  V extends [infer H, ...infer T]
  ? T['length'] extends 0
    ? [H, ...R]
    : Reverse<T, [H, ...R]>
...
export type Eval<A, env = [[]], prev = 0, Vscope extends boolean = false > =
  A extends Sexpr
  ...
  : A extends Atom
   ...
   : A extends Sym & [`sym`, infer SS]
     ? ReadLet<SS, env> extends infer U
       ? U extends Atom
         ? U
         : U extends TNotMatch // built-in fns are not matched so pick them up here.
           ? A // doing
           : [`prim`, U]
