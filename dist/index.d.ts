...
type ErrorCase<Case extends string, Msg extends string, S, Env = []> = {
    error: Case;
    message: Msg;
    sexpr: S;
} & (Env extends [] ? {} : {
...
type Reading<AS, EnvLifo = [[]], prev = 0, R = []> = R extends Atom[] ? AS extends [infer H, ...infer T] ? H extends Sym & ['sym', infer _ extends BuiltinsUnion] ? Reading<T, EnvLifo, prev, [...R, H]> : H extends Atom ? Reading<T, EnvLifo, prev, [...R, ReadAtom<H, EnvLifo, prev>]> : H extends Sexpr | LetForm ? Reading<T, EnvLifo, prev, [...R, Eval<H, EnvLifo, prev>]> : ErrorCase<ReadingError1, "", AS> : R : ErrorCase<ReadingError0, 'sexpr is not atom list.', R>;
...
export type LispIsEvery<S> = S extends [infer f, infer vs] ? vs extends ['vec'] ? ['prim', false] : Eval<[['sym', '='], [['sym', 'filter'], f, vs], vs]> : ErrorCase<EveryError0, 'every should have 2 args.', S>;
